...
public class Glyph {
	...
	public boolean drawCentered(World world, BlockPos pos, double spacing, GlyphOrientation orientation) {
	...
	private boolean draw(World world, BlockPos pos, double spacing, GlyphOrientation orientation, boolean centered) {
			...
			if (world.isRemote) {
				...
				for (int x = 0; x < imageProperties.width; x++) {
					for (int y = 0; y < imageProperties.height; y++) {
						...
						if (orientation == GlyphOrientation.VERTICAL) {
							posX = pos.getX() - (x * spacing) + (imageProperties.width * spacing) + centerModifier;
							...
							posX = pos.getX() - (x * spacing) + (imageProperties.width * spacing) + centerModifier;
							posY = pos.getY();
							posZ = pos.getZ() - (y * spacing) + (imageProperties.height * spacing) + centerModifier;
						}
