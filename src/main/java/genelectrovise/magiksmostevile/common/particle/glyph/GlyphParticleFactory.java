...
public class GlyphParticleFactory implements IParticleFactory<GlyphParticleData> { // IParticleFactory
	...
	@Nullable
	@Override
	public Particle makeParticle(GlyphParticleData glyphParticleData, World world, double xPos, double yPos, double zPos, double xVelocity, double yVelocity, double zVelocity) {
		...
		try {
			...
			newParticle.selectSpriteRandomly(sprites); // choose a random sprite from the available list (in this case there is only
														// one)
			return newParticle;
		} catch (Exception e) {
			e.printStackTrace();
