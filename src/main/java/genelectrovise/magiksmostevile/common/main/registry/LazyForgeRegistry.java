...
/**
 * Implementation of {@link IForgeRegistry} that delegates to an
 * {@link IForgeRegistry} instance that's lazily loaded from
 * {@link GameRegistry}.
 *
 * @author Choonster
 */
public class LazyForgeRegistry<V extends IForgeRegistryEntry<V>> implements IForgeRegistry<V> {
	...
	protected LazyForgeRegistry(final Class<V> registryType) {
		registry = new LazyValue<>(() -> Objects.requireNonNull(GameRegistry.findRegistry(registryType), () -> String.format("Registry of type %s not present", registryType.getName())));
	}
