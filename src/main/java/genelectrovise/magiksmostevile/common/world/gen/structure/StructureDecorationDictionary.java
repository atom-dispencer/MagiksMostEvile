...
public class StructureDecorationDictionary extends HashMap<Block, BlockReplacementList> {
  ...
  /**
   * Uses <code>super.put(key, value)</code> to add to the extended {@link TreeMap}, but returns
   * this object instead to make this chainable.
   * 
   * @param key
   * @param value
   * @return this
  ...
  /**
   * If the {@link Block} of the given {@link BlockState} is a key in this {@link TreeMap}, replace
   * it using the bound {@link BlockReplacementList}.
   * 
   * @param state
   * @return If success, the new {@link BlockState}. If failed, the old {@link BlockState}
   */
  public BlockState decorate(BlockState state, BlockPos pos) {
    ...
    BlockState newState;
    /*
     * if (state.getBlock() instanceof StairsBlock) { Main.LOGGER.debug("Is a StairsBlock!");
     * newState = list.get(selection - 1).block.getDefaultState().with(StairsBlock.FACING,
     * state.get(StairsBlock.FACING)); } else { Main.LOGGER.debug("Is not a StairsBlock"); newState
     * = list.get(selection - 1).block.getDefaultState(); }
     */
