...
/**
 * Used to decorate structures with random blocks based on a palette and
 * replacement conditions given using
 * 
...
public class StructureDecorator {
	...
	/**
	 * Decorates throughout the {@link MutableBoundingBox} specified in the
	 * constructor of this class using the {@link StructureDecorationDictionary}
	 * also given there.
	 * 
	 * @param type
	 */
		...
		for (int x = bounds.minX; x < bounds.maxX + 1; x++) {
			for (int y = bounds.minY; y < bounds.maxY + 1; y++) {
				for (int z = bounds.minZ; z < bounds.maxZ + 1; z++) {
					...
					if (dictionary.containsKey(oldState.getBlock()) || oldState.getBlock() instanceof ChestBlock) {
