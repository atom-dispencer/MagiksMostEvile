...
public class ShrinePiece {
	...
	public static class ShrineCentrePiece extends TemplateStructurePiece {
		...
		public boolean func_225577_a_(IWorld worldIn, ChunkGenerator<?> generator, Random rand, MutableBoundingBox mutableBB, ChunkPos pos) {
			...
			int groundHeight = worldIn.getHeight(Heightmap.Type.WORLD_SURFACE_WG, blockpos1.getX(), blockpos1.getZ());
			...
			decorateShrineWithBiome(worldIn, template.getMutableBoundingBox(placementSettings, pos.asBlockPos().add(0, worldIn.getHeight(Heightmap.Type.WORLD_SURFACE_WG, blockpos1.getX(), blockpos1.getZ()), 0)), pos, rand, lowerY, upperY);

			return superReturn;
		}

		private void decorateShrineWithBiome(IWorld worldIn, MutableBoundingBox mutableBB, ChunkPos pos, Random rand, int lowerY, int upperY) {
			StructureDecorator decorator = new StructureDecorator(mutableBB, worldIn, new ShrineDecorationDictionary(rand, worldIn.getBiome(pos.asBlockPos()), worldIn), lowerY, upperY);
