...
public class EnderParticleMessageHandlerOnClient {
	 ...
	 */
	public static void onMessageReceived(final EnderParticleMessageToClient message, Supplier<NetworkEvent.Context> ctxSupplier) {
	...
	private static void processMessage(EnderParticleMessageToClient message, Supplier<NetworkEvent.Context> ctxSupplier) {
		Random random = new Random();

		for (int i = 0; i < message.getCount(); i++) {
			Minecraft.getInstance().player.worldClient.addParticle(ParticleTypes.PORTAL, true, message.getPosition().getX(), message.getPosition().getY() + random.nextDouble() * 2.0D, message.getPosition().getZ(), random.nextGaussian(), 0.0D, random.nextGaussian());
		}
	}
