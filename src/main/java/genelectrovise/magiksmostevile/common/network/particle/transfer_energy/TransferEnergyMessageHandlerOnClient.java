...
public class TransferEnergyMessageHandlerOnClient {
	 ...
	 */
	public static void onMessageReceived(final TransferEnergyMessageToClient message, Supplier<NetworkEvent.Context> ctxSupplier) {
	...
	private static void processMessage(TransferEnergyMessageToClient message, Supplier<NetworkEvent.Context> ctxSupplier) {
		Vec3d crystal = new Vec3d(message.getDeparture().getX(), message.getDeparture().getY(), message.getDeparture().getZ());
		Vec3d altar = new Vec3d(message.getDestination().getX(), message.getDestination().getY(), message.getDestination().getZ());
		...
		Minecraft.getInstance().player.worldClient.addParticle(ParticleTypes.TOTEM_OF_UNDYING, true, message.getDeparture().getX() + 0.5, message.getDeparture().getY() + 0.5, message.getDeparture().getZ() + 0.5, direction.x * mul, direction.y * yMul, direction.z * mul);
	}
