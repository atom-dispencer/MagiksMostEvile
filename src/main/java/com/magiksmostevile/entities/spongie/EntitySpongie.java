...
import com.magiksmostevile.entities.spongie.SpongieEvent.SpongieSummonAidEvent;
...
public class EntitySpongie extends EntityMob
{
    protected static final IAttribute field_110186_bp = (new RangedAttribute("spongie.spawnReinforcements", 0.0D, 0.0D, 1.0D)).setDescription("Spawn Reinforcements Chance");
    private static final UUID babySpeedBoostUUID = UUID.fromString("B9766B59-9566-4402-BC1F-2EE2A276D836");
    private static final AttributeModifier babySpeedBoostModifier = new AttributeModifier(babySpeedBoostUUID, "Baby speed boost", 0.5D, 1);
    private final EntityAIBreakDoor field_146075_bs = new EntityAIBreakDoor(this);
    /** Ticker used to determine the time remaining for this spongie to convert into a villager when cured. */
    ...
    public EntitySpongie(World p_i1745_1_)
    {
        super(p_i1745_1_);
    ...
    public static SpongieSummonAidEvent fireSpongieSummonAid(EntitySpongie spongie, World world, int x, int y, int z, EntityLivingBase attacker, double summonChance)
    {
    	SpongieSummonAidEvent summonEvent = new SpongieSummonAidEvent(spongie, world, x, y, z, attacker, summonChance);
    ...
    /**
     * Set whether this spongie is a child.
     */
    ...
    /**
     * Return whether this spongie is a villager.
     */
    ...
    /**
     * Set whether this spongie is a villager.
     */
    ...
    /**
     * Called frequently so the entity can update its state every tick as required. For example, spongies and skeletons
    ...
    public boolean attackEntityFrom(DamageSource p_70097_1_, float p_70097_2_)
    {
        ...
        else
        {
            ...
            SpongieSummonAidEvent summonAid = fireSpongieSummonAid(this, worldObj, i, j, k, entitylivingbase, this.getEntityAttribute(field_110186_bp).getAttributeValue());
            ...
            {
                EntitySpongie entityspongie;
                if (summonAid.customSummonedAid != null && summonAid.getResult() == Result.ALLOW)
                {
                    entityspongie = summonAid.customSummonedAid;
                }
                else
                {
                    entityspongie = new EntitySpongie(this.worldObj);
                }
                ...
                for (int l = 0; l < 50; ++l)
                {
                    ...
                    {
                        entityspongie.setPosition((double)i1, (double)j1, (double)k1);

                        if (this.worldObj.checkNoEntityCollision(entityspongie.boundingBox) && this.worldObj.getCollidingBoundingBoxes(entityspongie, entityspongie.boundingBox).isEmpty() && !this.worldObj.isAnyLiquid(entityspongie.boundingBox))
                        {
                            this.worldObj.spawnEntityInWorld(entityspongie);
                            if (entitylivingbase != null) entityspongie.setAttackTarget(entitylivingbase);
                            entityspongie.onSpawnWithEgg((IEntityLivingData)null);
                            this.getEntityAttribute(field_110186_bp).applyModifier(new AttributeModifier("spongie reinforcement caller charge", -0.05000000074505806D, 0));
                            entityspongie.getEntityAttribute(field_110186_bp).applyModifier(new AttributeModifier("spongie reinforcement callee charge", -0.05000000074505806D, 0));
                            break;
                        }
                    }
                }
    ...
    {
        return "mob.spongie.death";
    }

    protected void func_145780_a(int p_145780_1_, int p_145780_2_, int p_145780_3_, Block p_145780_4_)
    {
        this.playSound("mob.spongie.step", 0.15F, 1.0F);
    }
    ...
    public void onKillEntity(EntityLivingBase p_70074_1_)
    {
        ...
        if ((this.worldObj.difficultySetting == EnumDifficulty.NORMAL || this.worldObj.difficultySetting == EnumDifficulty.HARD) && p_70074_1_ instanceof EntityVillager)
        {
            ...
            EntitySpongie entityspongie = new EntitySpongie(this.worldObj);
            entityspongie.copyLocationAndAnglesFrom(p_70074_1_);
            this.worldObj.removeEntity(p_70074_1_);
            entityspongie.onSpawnWithEgg((IEntityLivingData)null);
            entityspongie.setVillager(true);

            if (p_70074_1_.isChild())
            {
                entityspongie.setChild(true);
            }
    ...
    public IEntityLivingData onSpawnWithEgg(IEntityLivingData p_110161_1_)
    {
        ...
        {
            p_110161_1_1 = new EntitySpongie.GroupData(this.worldObj.rand.nextFloat() < ForgeModContainer.zombieBabyChance, this.worldObj.rand.nextFloat() < 0.05F, null);
        }

        if (p_110161_1_1 instanceof EntitySpongie.GroupData)
        {
            EntitySpongie.GroupData groupdata = (EntitySpongie.GroupData)p_110161_1_1;
        ...
        if (d0 > 1.0D)
        {
            this.getEntityAttribute(SharedMonsterAttributes.followRange).applyModifier(new AttributeModifier("Random spongie-spawn bonus", d0, 2));
        }

        if (this.rand.nextFloat() < f * 0.05F)
        {
            this.getEntityAttribute(field_110186_bp).applyModifier(new AttributeModifier("Leader spongie bonus", this.rand.nextDouble() * 0.25D + 0.5D, 0));
            this.getEntityAttribute(SharedMonsterAttributes.maxHealth).applyModifier(new AttributeModifier("Leader spongie bonus", this.rand.nextDouble() * 3.0D + 1.0D, 2));
    ...
    /**
     * Starts converting this spongie into a villager. The spongie converts into a villager after the specified time in
     * ticks.
     */
    ...
    public void handleHealthUpdate(byte p_70103_1_)
    {
        if (p_70103_1_ == 16)
        {
            this.worldObj.playSound(this.posX + 0.5D, this.posY + 0.5D, this.posZ + 0.5D, "mob.spongie.remedy", 1.0F + this.rand.nextFloat(), this.rand.nextFloat() * 0.7F + 0.3F, false);
        }
        else
        {
    ...
    /**
     * Returns whether this spongie is in the process of converting to a villager
     */
    ...
    /**
     * Convert this spongie into a villager.
     */
