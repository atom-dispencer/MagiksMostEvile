...
public class EntityAmethystSlime extends EntityLiving implements IMob
{
    ...
    public EntityAmethystSlime(World p_i1742_1_)
    {
        super(p_i1742_1_);
    ...
    protected EntityAmethystSlime createInstance()
    {
        return new EntityAmethystSlime(this.worldObj);
    }
     ...
     */
    public void setDead(boolean attacker)
    {
        ...
        if (!this.worldObj.isRemote && i > 1 && this.getHealth() <= 0.0F)
        {
            ...
            for (int k = 0; k < j; ++k)
            {
                ...
                EntityAmethystSlime entityslime = this.createInstance();
    ...
    /**
     * Returns true if the mob was killed by player. ADDED BY ME 
     */
    public static boolean wasEntityKilledByPlayer(DamageSource source){
    	...
    	}else if(attacker instanceof IThrowableEntity){
    		IThrowableEntity throwable = (IThrowableEntity)attacker;
    		attacker = throwable.getThrower();
    	}else if(attacker instanceof EntityThrowable){
    		EntityThrowable throwable = (EntityThrowable)attacker;
    		attacker = throwable.getThrower();
    	}
    ...
    protected Item getDropItem()            //EDITED BY ME FROM ENTITYSLIME
    {
    	//return this.getSlimeSize() == 1 ? Items.slime_ball : Item.getItemById(0);
    	int amethystSlimeToDrop = 1 + (int)(Math.random() * ((171 - 1) + 1));
		return Item.getItemById(amethystSlimeToDrop);
